...
const Register = () => {
  ...
  const validateInput = () => {
    ...
    const emailPattern = /^[a-zA-Z0-9._%+-]+@msquireit\.co.uk$/; // Restrict to middlewaretalents.com
    ...
    if (!email) {
      ...
      setEmailError("* Email must end with @msquireit.co.uk");
      isValid = false;
    } else {
      setEmailError("");
    }
  ...
  async function save(event) {
    ...
    if (!passwordPattern.test(password)) {
      setPasswordError(
        "* Password must be at least 7 characters long, contain at least one special character, and have one number."
      );
      return;
    } else {
      setPasswordError("");
    ...
    try {
      ...
      const response = await axios.post(
        `${url}/api/v1/employeeManager/register`,
        formData,
        {
          headers: {
      ...
      <div className="container">
        <main className="signup-container">
          <h1 className="heading-primary">
            Sign up<span className="span-blue">.</span>
          </h1>
          ...
          <form className="signup-form" onSubmit={save}>
            ...
            {employeeNameError && (
              <p className="error-message">{employeeNameError}</p>
            )}
